...
-(instancetype)initWithNomarImageArr:(NSArray *)nomarImageArr andSelectImageArr:(NSArray *)selectImageArr andtitleArr:(NSArray *)titleArr
{
    ...
    if (self) {
        //会话聊天
        ConversationListController *oneVC = [[ConversationListController alloc]init];
        //好友通讯录
        FriendsVC *twoVC = [[FriendsVC alloc]init];
        GameVC *threeVC= [[GameVC alloc]init];
        ShareVC *fourVC = [[ShareVC alloc]init];
        MyCenterVC *fiveVC = [[MyCenterVC alloc]init];
        ...
        {
            RedNavigationController *navi = [[RedNavigationController alloc]initWithRootViewController:VCArr[i]];
            ...
            // 导航栏字体的颜色
            [navi.navigationBar setTitleTextAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:19],NSForegroundColorAttributeName:[UIColor whiteColor]}];
